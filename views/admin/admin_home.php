...
if (isset($_GET['export']) && $_GET['export'] == 1) {
    ...
    $exportData = [
        'year' => date('Y'),
        'totalRevenue' => (float)$result['ukupni_prihod'], 
        'monthlyRevenue' => [],
        'salesByYear' => [],
        'users' => [
            'active' => $activeUsers,
        ...
        'bestSellingCourse' => [
            'name' => $topCourse['kurs_naziv'],
            'category' => $topCourse['kategorija'],
            'soldCopies' => (int)$topCourse['broj_prodatih'],
            'price' => (float)$topCourse['cena']
        ]
    ];

    
    foreach ($monthlyRevenueData as $row) {
        $monthName = $monthNames[$row['mesec'] - 1];
        $exportData['monthlyRevenue'][$monthName] = (float)$row['prihod'];
    }

    
    foreach ($salesData as $row) {
        $exportData['salesByYear'][$row['godina']] = (int)$row['broj_prodatih'];
    }

        header('Content-Type: application/json');
    header('Content-Disposition: attachment; filename="overview_export.json"');
